...
export const CharacterSelect: React.FC = () => {
  ...
  return (
    <div
      className="min-h-screen creamyBg text-black p-8"
      style={{
    ...
    >
      ...
      <div className="max-w-4xl mx-auto">
        <div className="grid grid-cols-2 gap-8">
          <div>
            <h2 className="text-2xl font-bold mb-6">Choose Your Background</h2>
            <div className="space-y-4">
              {Object.entries(backgrounds).map(([key, { name, description }]) => (
                <button
                  key={key}
                  onClick={() => setSelectedBackground(key as Background)}
                  className={`w-full text-left p-4 rounded-lg transition-all text-white ${
                    selectedBackground === key
                      ? 'bg-orange-600 shadow-lg'
                      : 'bg-slate-700 shadow-sm hover:bg-slate-300'
                  }`}
                >
                  <h3 className="font-bold text-lg">{name}</h3>
                  <p className={`text-sm ${selectedBackground === key ? 'text-orange-100' : 'text-gray-600'}`}>
                    {description}
                  </p>
                </button>
              ))}
          ...
          <div>
            <h2 className="text-2xl font-bold mb-6">Choose Your Alignment</h2>
            <div className="space-y-4">
                ...
                <button
                  ...
                  className={`w-full text-left p-4 rounded-lg transition-all text-white ${
                    selectedAlignment === key
                      ? 'bg-orange-600 shadow-lg'
                      : 'bg-slate-7 shadow-sm hover:bg-slate-300'
                  }`}
                >
                  <h3 className="font-bold text-lg">{name}</h3>
                  <p className={`text-sm ${selectedAlignment === key ? 'text-orange-100' : 'text-gray-600'}`}>
                    {description}
                  </p>
                </button>
              ))}
        ...
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="px-8 py-4 bg-orange-600 text-white rounded-lg text-lg font-bold hover:bg-orange-700 transition-colors"
          >
            Begin Your Journey
          </button>
        </div>
